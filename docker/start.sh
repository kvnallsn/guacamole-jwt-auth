...
FATAL: Missing required environment variables
-----------------------------------------------------------------------------------
If using the JWT authentication extension, you must provide JWT_HEADER or JWT_COOKIE.
If both are provided, JWT_HEADER will take precedence over JWT_COOKIE.

    JWT_HEADER          The name of the header storing the JWT

    JWT_COOKIE          The name of the HTTP cookie storing the JWT

Additionally, you must also provide the following environment variables in all cases:

    JWT_JWKS_URL        The URL to download the JSON Web Keys from

    JWT_AUDIENCE        The value of the 'aud' claim in the JWT 
END
        exit 1;
    fi

    # Update config file
    set_property            "jwt-jwks-url"      "$JWT_JWKS_URL"
    set_property            "jwt-audience"      "$JWT_AUDIENCE"
    set_optional_property   "jwt-header"        "$JWT_HEADER"
    set_optional_property   "jwt-cookie"        "$JWT_COOKIE"
    set_optional_property   "jwt-jwks-path"     "$JWT_JWKS_PATH"
    set_optional_property   "jwt-user-claim"    "$JWT_USER_CLAIM"
    set_optional_property   "jwt-roles-claim"   "$JWT_ROLES_CLAIM"
    set_optional_property   "jwt-custom-claims" "$JWT_CUSTOM_CLAIMS"

    # Add required .jar files to GUACAMOLE_EXT
    ln -s /opt/guacamole/jwt/guacamole-auth-*.jar   "$GUACAMOLE_EXT"
}
